...
public $textarea_lineheight;
    ...
    public function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
    ...
    public function print_ob_button($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        // BUTTON
             if ($this->mpdf->useActiveForms) {
                 // Flags: 1 - Readonly; 3 - No export; 
...
public function _putRadioItems($n)
{
    ...
    public function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = array(), $align = 'L', $hidden = false, $maxlen = -1, $js = '', $background_col = false, $border_col = false)
    {
        // Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 - Password
    ++$this->formn;
        if ($align == 'C') {
    ...
    // CHECKBOX
    public function SetCheckBox($w, $h, $name, $value, $title = '', $checked = false, $flags = array(), $disabled = false)
    {
    ...
    // RADIO
    public function SetRadio($w, $h, $name, $value, $title = '', $checked = false, $flags = array(), $disabled = false)
    {
        $this->SetFormButton($w, $h, $name, $value, 'radio', $title, $flags, $checked, $disabled);
        $this->mpdf->x += $w;
    }

    public function SetFormReset($w, $h, $name, $value = 'Reset', $title = '', $flags = array(), $background_col = false, $border_col = false, $noprint = false)
    {
        if (!$name) {
            $name = 'Reset';
        }
    ...
    public function SetJSButton($w, $h, $name, $value, $js, $image_id = 0, $title = '', $flags = array(), $indexed = false, $background_col = false, $border_col = false, $noprint = false)
    {
    ...
    public function SetFormSubmit($w, $h, $name, $value = 'Submit', $url, $title = '', $typ = 'html', $method = 'POST', $flags = array(), $background_col = false, $border_col = false, $noprint = false)
    {
        if (!$name) {
            $name = 'Submit';
        }
    ...
    public function SetFormButton($bb, $hh, $name, $value, $type, $title = '', $flags = array(), $checked = false, $disabled = false, $background_col = false, $border_col = false, $noprint = false)
    {
        ++$this->formn;
        if (!preg_match('/^[a-zA-Z0-9_:\-]+$/', $name)) {
            $this->mpdf->Error('Field ['.$name.'] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)');
