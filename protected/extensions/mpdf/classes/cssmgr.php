...
//print_r($this->cascadeCSS); exit;
    ...
    public function _fix_borderStr($bd)
    {
        ...
        // #000000
        elseif (is_array($this->mpdf->ConvertColor($prop[0]))) {
            $c = $prop[0];
        }
        // 1px 
        else {
...
public function array_merge_recursive_unique($array1, $array2)
{
    ...
    public function _mergeFullCSS($p, &$t, $tag, $classes, $id)
    {
                ...
                if (preg_match('/'.$tag.'>>SELECTORNTHCHILD>>(.*)/', $k, $m)) {
                    ...
                    if ($tag == 'TR') {
                        ...
                        if ($m[1] == 'ODD' && ($this->mpdf->col % 2) == 0) {
                            $select = true;
                        } elseif ($m[1] == 'EVEN' && ($this->mpdf->col % 2) == 1) {
                            $select = true;
                        } elseif (preg_match('/(\d+)N\+(\d+)/', $m[1], $a)) {
...
/*-- END LISTS --*/
    ...
    if ($inherit == 'BLOCK') {
        ...
        } elseif (isset($biilp[ 'tolower' ]) && $biilp[ 'tolower' ]) {
            $p['TEXT-TRANSFORM'] = 'lowercase';
        } elseif (isset($biilp[ 'capitalize' ]) && $biilp[ 'capitalize' ]) {
    ...
    if ($inherit == 'TOPLIST') {
        if ($this->listCSSlvl == 1) {
            ...
            if (isset($bilp[ 'toupper' ]) && $bilp[ 'toupper' ]) {
                $p['TEXT-TRANSFORM'] = 'uppercase';
            } elseif (isset($bilp[ 'tolower' ]) && $bilp[ 'tolower' ]) {
                $p['TEXT-TRANSFORM'] = 'lowercase';
            } elseif (isset($bilp[ 'capitalize' ]) && $bilp[ 'capitalize' ]) {
                $p['TEXT-TRANSFORM'] = 'capitalize';
            }
