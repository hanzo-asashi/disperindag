...
class SandboxBehavior extends CActiveRecordBehavior
{
     ...
     * 
     * @param int|CActiveRecord $revision revision ID or record
     * 
     ...
     * interface.
     * 
     * @return bool whether owner is revisionable
     */
     ...
     * 
     * @param int $revisionId revision id to find
     * 
     ...
     * 
     * @param int $revisionType revision type
     * 
     * @return revision object or null if revision not found
     */
     ...
     * 
     * @return array|bool False if publishing should not be executed. Array of 
     ...
     * 
     * @return bool whether reverting should be processed
     */
     ...
     * 
     * @return int publishing result
     */
    protected function publishWithHistory($newAttributes = array())
    {
        $owner = $this->getOwner();
        $newRevision = $this->createRevision(self::PUBLISHED_REVISION);
        $oldPublished = $owner->published;
        //if (!$oldPublished) $oldPublished = $this->findPublished();

        if ($oldPublished) {
            $oldPublished->{$this->revisionTypeField} = self::HISTORY_REVISION;
    ...
    protected function publishSandbox($newAttributes = array())
    {
        ...
        if (!$published) {
            $published = $this->createRevision(self::PUBLISHED_REVISION);
        }
        $ret = $this->copyRevision(
            $owner->sandbox,
            $published,
    ...
    {
        $this->getOwner()->sandbox = $this->createRevision(self::SANDBOX_REVISION);
    }
     ...
     * Create new revision.
     * 
     * @param int $revisionType new revision type
     * 
     ...
     *
     * @return bool whether the record is published
     */
